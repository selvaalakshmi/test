    ...
    */
    ...
    public static function getSubscribedEvents()
    {
        return [
            ...
            'Enlight_Controller_Action_PostDispatchSecure_Widgets_Listing' => 'onPostDispatchWidgetsListing'
        ];
    }

    public function onPostDispatchWidgetsListing(\Enlight_Event_EventArgs $args)
    {
        ...
        if($action == 'getCategory'){
            ...
            foreach ($supplierCategories as $supplierCategorie){
                $supplier = $this->getMappedSupplierByParentCategory($categoryId, $supplierCategorie);
                if($supplier){
                    $suppliers[] = $supplier;
                }
            }
            ...
            if($suppliers){
                ...
                $view->parentCategoryId = $categoryId;

                $category = $view->category;
                $category['children'] = [0]; // execute block one time
    ...
    public function onPostDispatch(\Enlight_Event_EventArgs $args)
    {
        ...
        $menu = $this->getAdvancedMenu($parent, $categoryId, 4);
...
//        $supplier['attribute'] = $supplierObj->getAttribute();
    ...
    private function getTopArticles($categoryId, $limit = 5)
    {
        ...
        $sql = "SELECT a.id, a.description
                FROM s_categories a
                INNER JOIN s_articles_categories_ro ac ON (a.id = ac.categoryID)
