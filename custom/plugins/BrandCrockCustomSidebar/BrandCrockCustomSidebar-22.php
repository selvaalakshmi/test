...
class BrandCrockCustomSidebar extends Plugin
{
    ...
    */
    ...
    private function getSubofSubCategories()
    {
        $uri = $_SERVER['REQUEST_URI'];
        if ($uri) {
            $sCategoryCheck = strpos($uri, 'sCategory/');
            if ($sCategoryCheck) {
                ...
                $queryString = strpos($parentCatId, '?');
                if ($queryString) {
                    $ret = explode('?', $parentCatId);
                    $ret = $ret[0];
                    ...
                    foreach ($categories as $category ){
                        ...
                        //echo '<pre>';
                        //echo $catId;
                        $sql = "select id, description as name from s_categories where parent = {$catId} order by id;";
                        $categorie = Shopware()->Db()->fetchAll($sql);
